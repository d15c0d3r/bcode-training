...
export default (() => {
  ...
  function doStuff(value: any): void {
    ...
    if (typeof value === "string" || typeof value === "number") value; // hover over `value` here
  }
  ...
  const numbers: numORnumArr[] = [1, 2, 3, [44, 55], 6, [77, 88], 9, 10];

  function flatten(array: numORnumArr[]): number[] {
    const flattened = [];
  ...
  function interrogateAnimal(animal = getRandomAnimal()) {
    ...
    else animal.fly(10); // call only if it is a bird
